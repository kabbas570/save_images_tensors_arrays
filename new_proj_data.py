...
def generate_label_3_proj(gt):
        ...
        temp_[0,:][np.where((gt>0.5) & (gt <1.5))]=1
        temp_[1,:][np.where((gt>1.5) & (gt <2.5))]=1
        temp_[2,:][np.where((gt>2.5) & (gt <3.2))]=1
...
class Dataset_V(Dataset): 
    ...
    def __getitem__(self, index):
        ...
        LA_ES_img = np.expand_dims(img, axis=0)
        ...
        org_dim2 = img.shape[2] 
        img = Cropping_3d(org_dim3,org_dim1,org_dim2,DIM_,img) 
        LA_ED_img = np.expand_dims(img, axis=0)
        ...
        temp_LA_ES = generate_label_3_gt(img) 
        ...
        img = Cropping_3d(org_dim3,org_dim1,org_dim2,DIM_,img) 
        img = np.expand_dims(img, axis=0)
        temp_LA_ED = generate_label_3_gt(img) 
        ...
        return LA_ES_img[0,:,:],temp_LA_ES[:,0,:],proj_2d_ES,LA_ED_img[0,:,:],temp_LA_ED[:,0,:],proj_2d_ED,self.images_name[index]
